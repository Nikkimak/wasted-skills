...
For `standard` or `deep` reviews, invoke the `cross-model-review` skill (via the Skill tool, `/skill:cross-model-review`) against the security-relevant PRD, implementation, and task drafts, using the security questions in the checklist as reviewer context. Reuse its live Kimi/Claude loop, but apply this skill's stricter stop rules: models cannot accept material risk for the human.
