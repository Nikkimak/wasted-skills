...
8. Invoke `$cross-model-review` with the `task-plan` profile. Correct supported findings, resume the same Claude reviewer session for full recheck, and surface all remaining material/minor differences to the human.
9. Obtain human approval of the task graph, then invoke `$feature-security-review` before any tracker publication.
