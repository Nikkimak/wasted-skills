...
Read `references/review-profiles.md`, select `prd`, `implementation`, or `task-plan`, and use `scripts/claude_review.py`. `task-plan` is explicit-request only and authoring skills must not offer it routinely. Unless the human chooses otherwise, use `claude-opus-4-8` with `xhigh` effort for probe, review, and recheck; never change reviewer model or effort mid-session.
