...
description: Organize and clean up project documentation, workspace structure, and repo boundaries for a real coding project using the bundled canonical playbook as read-only guidance. Use when Kimi needs to do an initial bootstrap, one-time migration, or major structural cleanup of project docs; untangle messy canonical documentation; default to one project-root git repo unless the user explicitly wants runtime isolation; support a split model where `src/` has its own git repo, or an advanced multi-runtime split under `src/`; verify or reshape existing git boundaries; choose minimal versus standard versus full documentation scope; or upgrade an existing project to a clearer source-of-truth model. Do not use this skill for routine day-to-day edits in a project that is already organized.
---
...
- `${KIMI_SKILL_DIR}/references/llm-friendly-project-structure-playbook.md`
- `${KIMI_SKILL_DIR}/references/project-agents-template.md`
...
3. Read `${KIMI_SKILL_DIR}/references/git-topology-checklist.md` before choosing the repo model.
4. Read `${KIMI_SKILL_DIR}/references/mode-selection.md` to choose `minimal`, `standard`, or `full` after the repo model is understood.
5. Read `${KIMI_SKILL_DIR}/references/canonical-docs-matrix.md` to determine the required file set for the chosen mode.
6. Read `${KIMI_SKILL_DIR}/references/implementation-checklist.md` before making edits.
7. Use the templates in `${KIMI_SKILL_DIR}/assets/templates/` only for the files that are actually needed.
...
- `${KIMI_SKILL_DIR}/references/playbook-source.md`
...
- `${KIMI_SKILL_DIR}/references/mode-selection.md`
  Read when it is unclear how much of the documentation system should be materialized.
- `${KIMI_SKILL_DIR}/references/git-topology-checklist.md`
  Read when creating or verifying project-root git versus split `src/` git separation.
- `${KIMI_SKILL_DIR}/references/implementation-checklist.md`
  Read when executing the playbook on a project.
- `${KIMI_SKILL_DIR}/references/canonical-docs-matrix.md`
  Read when deciding which canonical docs are required, optional, or out of scope.
- `${KIMI_SKILL_DIR}/references/templates-policy.md`
  Read before stamping templates into a project.

## Templates

Use the templates in `${KIMI_SKILL_DIR}/assets/templates/` as accelerators, not as unconditional output.
