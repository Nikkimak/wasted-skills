...
7. Invoke the `cross-model-review` skill (via the Skill tool as `/cross-model-review`) with the `implementation` profile. Verify GPT findings against the repository, prepare supported corrections, and resume the same GPT reviewer session for a full-document recheck.
...
9. Mark the implementation design accepted only after explicit human approval. Remove an obsolete `WORK-HANDOFF.md` through the `feature-context-handoff` skill when canonical documents and the work plan make the next phase clear. End with a handoff to the `feature-delivery-plan` skill.
