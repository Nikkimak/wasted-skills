...
Invoke the `feature-security-review` skill (via the Skill tool, `/skill:feature-security-review`) only for `deep` risk or explicit human request and keep its durable result in this implementation document. After the document is complete, ask the human whether to run one `implementation`-profile pass of the `cross-model-review` skill (`/skill:cross-model-review`); never infer approval from security depth or from having run the security enrichment.
...
Use `implementation_doc_not_required` only when one bounded task can safely own all shared notes and no material shared security decision exists. Give the reason and constraints that the `feature-delivery-plan` skill (via the Skill tool, `/skill:feature-delivery-plan`) must preserve.
