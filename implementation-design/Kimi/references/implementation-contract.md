...
- `implementation_doc_not_required`: one bounded task can own all notes and no new or changed shared contract/security decision exists. Give the reason and the planning constraints that the `feature-delivery-plan` skill (via the Skill tool, `/skill:feature-delivery-plan`) must preserve.
