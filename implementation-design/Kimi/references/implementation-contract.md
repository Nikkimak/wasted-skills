...
Invoke the `feature-security-review` skill (via the Skill tool, `/skill:feature-security-review`) only for `deep` risk or explicit human request. Keep its durable result in this implementation document, then run one `implementation`-profile pass of the `cross-model-review` skill (via the Skill tool, `/skill:cross-model-review`) for the combined technical and security contract.
