...
Invoke `$feature-security-review` only for `deep` risk or explicit human request. Keep its durable result in this implementation document, then use one `$cross-model-review` `implementation` pass for the combined technical and security contract.
