...
- Put security controls into the design, while leaving final security readiness to `$feature-security-review` after task planning.
- List open technical choices explicitly for human resolution.

## No-document result

Use `implementation_doc_not_required` when all shared technical meaning fits safely in one bounded task contract. Give a short evidence-based reason and any constraints that `$feature-delivery-plan` must preserve.
