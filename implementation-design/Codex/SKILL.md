...
6. Invoke `$cross-model-review` with the `implementation` profile. Verify Claude findings against the repository, prepare supported corrections, and resume the same reviewer session for a full recheck.
...
8. Mark the implementation design accepted only after explicit human approval. End with a handoff to `$feature-delivery-plan`.
