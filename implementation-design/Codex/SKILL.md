...
When continuing an existing feature, use `$feature-context-handoff` to read any feature-local `WORK-HANDOFF.md` after mandatory repository instructions, then load its named accepted inputs and active implementation draft. Canonical artifacts override the handoff.

Before starting cross-model review, rely only on the Codex context/status indicator when available. If the full review-and-recheck loop cannot safely fit, checkpoint the coherent draft with `$feature-context-handoff` and recommend a fresh session. Do not estimate context numerically or install automatic monitoring.
...
Skip the document when one bounded task can safely carry its implementation notes without duplicating shared decisions. Record the explicit `implementation_doc_not_required` conclusion and why, then hand off to `$feature-delivery-plan`.
...
3. Ask the human only about material technical/product tradeoffs that cannot be resolved from canonical evidence. Return product changes to `$feature-design` rather than silently altering the PRD.
...
7. Invoke `$cross-model-review` with the `implementation` profile. Verify Claude findings against the repository, prepare supported corrections, and resume the same reviewer session for a full-document recheck.
...
9. Mark the implementation design accepted only after explicit human approval. Remove an obsolete `WORK-HANDOFF.md` through `$feature-context-handoff` when canonical documents and the work plan make the next phase clear. End with a workflow handoff to `$feature-delivery-plan`.

## Boundaries

- Do not rewrite business scope; return PRD changes to `$feature-design`.
...
Report either the accepted implementation document path or `implementation_doc_not_required`, the accepted PRD version it constrains, key shared technical decisions, remaining implementation constraints for planning, cross-model review outcome, and readiness for `$feature-delivery-plan`.
