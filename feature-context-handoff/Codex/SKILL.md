...
Create or refresh only when the human pauses incomplete work, a blocker stops it, or the next atomic checkpoint/review loop cannot safely finish in this session. Do not create routine handoffs when canonical state already makes the next phase obvious.
...
Use only a host-provided context indicator; ask the human to check `/status` or their statusline when it is not visible, and never infer a token count or percentage from conversation length. Before an atomic phase—especially initial review, corrections, human decisions, and recheck—create or refresh the handoff and recommend a fresh session when capacity is doubtful. This skill installs no hooks, timers, plugins, or automatic termination.
...
For `resume`, report the verified phase and exact action being continued. For `pause`, report the handoff path and tell the human that the safe continuation is a fresh Codex session in the same repository followed by “continue <feature>”. For `close`, report that the ephemeral handoff was removed and name the canonical source that now carries the next state.
