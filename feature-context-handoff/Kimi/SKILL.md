...
Carry one unfinished feature into a fresh Kimi session without replaying its full conversation history. Keep durable truth in canonical feature documents; use `WORK-HANDOFF.md` only as a short-lived navigation and process-state pointer.
...
- the human and Kimi intentionally move an unfinished phase or its review loop to a fresh session.
...
Use only a host-provided context indicator as numerical evidence. In Kimi Code, inspect the context consumption shown by `/usage`; when only the human can see the indicator, ask them to check `/usage`. Never invent a remaining-token count or percentage from conversation length.

At a major boundary, especially before a complete cross-model review, decide whether the next atomic phase can finish with comfortable margin. A review includes the initial reviewer call, author corrections, human decisions, and a full reviewer recheck. If capacity is doubtful, create or refresh the handoff and recommend a fresh Kimi session in the same repository. A fresh session is preferable to restoring or forking an old session when the purpose is to discard old conversational context.
...
- Preserve relevant user changes and distinguish them from Kimi changes.
