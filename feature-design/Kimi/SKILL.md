...
8. Invoke the `cross-model-review` skill (via the Skill tool, `/skill:cross-model-review`) with the `prd` profile. Keep the current session as author, let Claude challenge the draft, prepare supported corrections, and recheck them in the same Claude session.
...
10. Mark the PRD accepted only after explicit human approval. End with a handoff to the `implementation-design` skill (via the Skill tool, `/skill:implementation-design`); do not continue into implementation or task planning in this skill.
