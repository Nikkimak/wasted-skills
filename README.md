...
2. `implementation-design` first decides whether a separate technical document is justified. A no-document result goes directly to human approval; a complete implementation design receives the same recommendation-and-consent decision before any combined technical/security `cross-model-review`. Exceptional payment, identity, privileged-secret, sensitive-user-data, isolation, destructive, or irreversible risk may invoke `feature-security-review` to deepen the draft without authorizing cross-model review.
...
1. Invoke `feature-design` while discussing or revising a feature. It reads canonical context, asks adaptive high-value questions, drafts only the PRD, then recommends for or against cross-model review and asks the human. It never invokes review before explicit approval.
...
3. Invoke `implementation-design` only after PRD approval. It either records that one bounded task needs no shared design or produces a security-aware technical contract. Only a produced document receives a review recommendation and consent question; a no-document result does not. Exceptional high-risk work invokes `feature-security-review` as local deep-design enrichment without granting review consent.
...
1. Invoke `feature-design` while discussing or revising a feature. It drafts only the PRD, recommends for or against cross-model review, and asks the human before invoking it.
...
3. Invoke `implementation-design` after PRD approval. It creates a security-aware contract only when justified; only a produced document receives a review recommendation and consent question. A no-document result goes directly to human approval. Exceptional high-risk work invokes `feature-security-review` as local deep-design enrichment without granting review consent.
4. Invoke `feature-delivery-plan` after technical/security approval. It maps accepted requirements and controls, conditionally creates `EXECUTION-READINESS.md`, runs local checks, and obtains human approval without offering routine GPT review.
5. Publish approved tracker drafts only through the project's bounded intake path on a later explicit action.
...
3. Invoke `implementation-design` only after PRD approval. It creates a security-aware technical contract only when justified; only a produced document receives a review recommendation and consent question. A no-document result goes directly to human approval. Exceptional high-risk work invokes `feature-security-review` as local deep-design enrichment without granting review consent.
4. Invoke `feature-delivery-plan` after product and technical/security approval. It creates the minimal vertical delivery graph, conditionally creates `EXECUTION-READINESS.md`, runs local checks, and obtains human approval without offering routine Claude review.
5. Publish approved tracker drafts only through the project's bounded intake path on a later explicit action.
